...
module SPI_mnrch ( 
    ...
    // read data logic
    // Capture the fully-shifted 16-bit word on the final shift event.
    // At (done_15 && shft_im), a shift will also occur this cycle; using
    // non-blocking semantics, shft_reg on the RHS would be the pre-shift
    // value. To include the last MISO bit, form the post-shift value
    // explicitly from shft_reg and the sampled miso_smpl.
    always_ff @(posedge clk or negedge rst_n) begin
        if(!rst_n) begin
            rd_data <= 16'b0;
        end else if(done_15 && shft_im) begin
            rd_data <= {shft_reg[14:0], miso_smpl};
        end
    end
endmodule