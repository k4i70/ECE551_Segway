...
task automatic apply_reset(input string tag);
	...
	wait_cycles(clk, 200);
	wait (rst_n === 1'b1);
	wait_cycles(clk, 5000);  // Allow time for inertial interface to start and A2D to do initial reads
...
task automatic too_fast_test();
	...
	if (!overspeed_seen) begin
		...
		failed_tests++;
		stop_forced_too_fast();  // Ensure forces released on early exit
		return;
	end
	...
	// Always ensure forces are released before exiting
	stop_forced_too_fast();
