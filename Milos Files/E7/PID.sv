...
module PID(
    ...
    parameter fast_sim = 1'b1; // Set to 1 for fast simulation mode
        ...
        if(fast_sim) begin : GEN_FAST_SIM
            always_ff @(posedge clk or negedge rst_n) begin
                if(!rst_n)
                    ...
                    long_tmr <= pwr_up ? (&long_tmr[26:19] ? long_tmr : long_tmr + 256) : 27'h0000000;
            end
    ...
    generate
        if(fast_sim) begin : GEN_FAST_I_TERM
            assign I_term = (|integrator[17:15] & ~&integrator[17:15]) ? (integrator[17] ? 15'h8000 : 15'h3FFF) : {integrator[15:1]};
        end else begin : GEN_NORMAL_I_TERM
