module SPI_iNEMO1(SS_n,SCLK,MISO,MOSI,INT);
  ...
  reg [12:0]update_period;		// around 3kHz
  reg clr_INT;
  ...
  always_ff @(negedge SCLK, negedge POR_n)
    if (!POR_n)
	  ...
	  write_reg <= 1'b1;
	else if (write_reg && SS_n)		// clear write_reg after SS_n goes high
	  write_reg <= 1'b0;
  ...
  ////////////////////////////////////////////////
  always_ff @(posedge SS_n, negedge POR_n)
    if (!POR_n) begin
      // Initialize registers array to all zeros on reset
  ...
  // (This block was duplicate - removed)         //
  ////////////////////////////////////////////////
